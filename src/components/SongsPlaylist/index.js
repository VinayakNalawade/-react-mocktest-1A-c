...
class SongsPlaylist extends Component {
  state = {search: '', trackList: initialTracksList}
  ...
  deleteTrack = id => {
    ...
    const newList = trackList.filter(each => each.id !== id)

    this.setState({trackList: newList})
  }

  renderPlaylist = () => {
    const {search, trackList} = this.state
    ...
    return (
      <Playlist>
        {filteredList.map(each => (
          <Track key={each.id} track={each} deleteTrack={this.deleteTrack} />
        ))}
      </Playlist>
    )
  }
  ...
  render() {
    ...
    return (
      <MainContainer>
        <TopSection>
          <TopHeading>Ed Sheeran</TopHeading>
          <TopPara>Singer</TopPara>
        </TopSection>
        <BottomSection>
          <PlaylistInput>
